...
class PresenceHandlerTestCase(BaseMultiWorkerStreamTestCase):
    ...
    @parameterized.expand(
        ...
        name_func=lambda testcase_func, param_num, params: (
            f"{testcase_func.__name__}_{param_num}_{'workers' if params.args[5] else 'monolith'}"
        ),
    )
    @unittest.override_config({"experimental_features": {"msc3026_enabled": True}})
    ...
    @parameterized.expand(
        ...
        name_func=lambda testcase_func, param_num, params: (
            f"{testcase_func.__name__}_{param_num}_{'workers' if params.args[4] else 'monolith'}"
        ),
    )
    @unittest.override_config({"experimental_features": {"msc3026_enabled": True}})
